...
export default function Pricing({
  ...
  const handleCheckout = async (price: Price) => {
    ...
    if (!user) {
      return router.push('/views/signin');
    }
    if (subscription) {
      return router.push('/views/pages/account');
    }
    try {
  ...
  return (
    <div>
      ...
      <div className="flex flex-col items-center my-12 space-y-4 sm:mt-8 sm:space-y-0 md:mx-auto md:max-w-2xl sm:grid sm:gap-6 sm:grid-cols-5">
        {logos.map((logo) => (
          <div key={logo.name} className="flex items-center justify-start">
            <a href={logo.path} aria-label={logo.name + ' Link'}>
              <Image
                src={logo.src}
                alt={logo.name + ' Logo'}
